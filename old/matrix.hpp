 ...
 *  \brief Module containing mask matrices
 */
...
namespace ev_matrix {
  ...
  class MaskMatrix {
  protected:
    /*! \typedef DynamicMatrix
     *  \brief Eigen/Dense dynamic array for a given datatype and a storage order
     */
    typedef typename Eigen::Array<data_type, Eigen::Dynamic, Eigen::Dynamic, StorageOrder> DynamicMatrix;
    // typedef typename Eigen::Matrix<data_type, Eigen::Dynamic, Eigen::Dynamic, StorageOrder> DynamicMatrix;
    // C-STYLE CONVENTION: [LOW, UP) -> INIT SHALL BE DONE CONSISTENTLY !
    int low_x, up_x, low_y, up_y;
    int dim_x, dim_y;
    DynamicMatrix data;
  public:
    // Add possibility to access via raw index
    ...
    int get_lx (void) const { return low_x; }
    int get_ly (void) const { return low_y; }
    int get_ux (void) const { return up_x; }
    int get_uy (void) const { return up_y; }
    int get_dim_x (void) const { return dim_x; }
    int get_dim_y (void) const { return dim_y; }
    ...
    // KEEP C-STYLE ARRAY ACCESSING ORDER [0,...,n-1]
    ...
    // Returns raw buffer
    DynamicMatrix& raw_buffer (void)
    {
      return data;
    }
    const DynamicMatrix& raw_buffer (void) const
    {
      return data;
    }
    // THIS MAY BE UNUSED
    ...
    MaskMatrix submatrix ( int lx, int ux, int ly, int uy ) const
    {
      MaskMatrix sub( lx, ux, ly, uy, data.block( idx_i(lx), idx_j(ly), ux-lx, uy-ly ) );
      return sub;
    }
    // These have sense as long as data_type is arithmetic ...
    // Need to add binary operators also!
    MaskMatrix& operator = (data_type a)
    {
      data = a;
      return *this;
    }
    ...
    MaskMatrix& operator /= (data_type a)
    {
      assert( (a!=0) && "Dividing matrix by zero" );
      data /= a;
      return *this;
    }
    MaskMatrix& operator += (data_type a)
    ...
    // Template function to copy-cast underlying Eigen data structure
    template <class rhs_type>
    MaskMatrix<data_type>& copy_cast (const MaskMatrix<rhs_type>& rhs)
    {
      data = (rhs.raw_buffer()).template cast<data_type>();
      return *this;
    }
  };
   ...
   *  \brief A mask matrix that needs to be centered around a point (cell)
   */
  template <class data_type, int StorageOrder = Eigen::RowMajor>
  ...
  public:
    ...
    /*
    SlideMaskMatrix(int nc):
      n_cut(nc), Base(-nc,nc+1,-nc,nc+1) { }
    SlideMaskMatrix(int nc, const data_type& dfl):
      n_cut(nc), Base(-nc,nc+1,-nc,nc+1,dfl) { }
    int get_n_cut(void) const { return n_cut; }
    */
    // Getter
    int get_n_cut_x(void) const { return n_cut_x; }
    int get_n_cut_y(void) const { return n_cut_y; }
  };
   ...
   *  \brief A mask matrix with the functionality of halo blocks
   */
  // NB Current implementation works iif nx_cut = ny_cut;
  template <class data_type, int StorageOrder = Eigen::RowMajor>
  ...
  public:
    ...
    HaloMaskMatrix( const MaskMatrix<data_type>& data_, int nc ):
      MaskMatrix<data_type>(data_), n_halo_x(nc), n_halo_y(nc) { set_schema(); }
    HaloMaskMatrix( const MaskMatrix<data_type>& data_, int nc_x, int nc_y ):
      MaskMatrix<data_type>(data_), n_halo_x(nc_x), n_halo_y(nc_y) { set_schema(); }
    // COPY ASSIGNMENT ETC ...
    ~HaloMaskMatrix() = default;
    // Getter
    int get_n_halo_x(void) const { return n_halo_x; }
    int get_n_halo_y(void) const { return n_halo_y; }
    // Placing mask matrix into halo matrix (the cast is needed)
    template <class rhs_type>
    HaloMaskMatrix<data_type>& copy_patch (const MaskMatrix<rhs_type>& rhs, int px, int py)
    {
      Base::data.block(Base::idx_i(px), Base::idx_j(py), rhs.get_dim_x(), rhs.get_dim_y())
        = (rhs.raw_buffer()).template cast<data_type>();
      return *this;
    }
    void set_outer_block ( int r, const DynamicMatrix& rhs )
    {
      Base::data.block(Base::idx_i(lx_recv[r]), Base::idx_j(ly_recv[r]),
        ux_recv[r]-lx_recv[r], uy_recv[r]-ly_recv[r]) = rhs;
    }
    DynamicMatrix get_inner_block ( int r )
    {
      return Base::data.block(Base::idx_i(lx_send[r]), Base::idx_j(ly_send[r]),
  ...
  public:
    MatrixConvolutioner(MaskMatrix<data_type>& r, const SlideMaskMatrix<data_type>& s,
      ...
      result(r), slider(s), base(b), n_halo_x(b.get_n_halo_x()), n_halo_y(b.get_n_halo_y()),
      n_cut_x(s.get_n_cut_x()), n_cut_y(s.get_n_cut_y()), lx(r.get_lx()), ly(r.get_ly()),
      ux(r.get_ux()), uy(r.get_uy()), def(d) {
    ...
    void convolute(int i, int j) {
      // Many of the for cycles can be resolved using Eigen operation overloading and expression templates...
      /*
      temp_result = def;
      ...
      */
      // DEBUG
      // # # # # #
      // std::cout << slider.raw_buffer().rows() << "x" << slider.raw_buffer().cols() << std::endl;
      // std::cout << base.submatrix(-n_cut_x+i,n_cut_x+i+1,-n_cut_y+j,n_cut_y+j+1).raw_buffer().rows()
      //   << "x" << base.submatrix(-n_cut_x+i,n_cut_x+i+1,-n_cut_y+j,n_cut_y+j+1).raw_buffer().cols() << std::endl;
      // # # # # #
      result(i,j) = ( slider.raw_buffer() *
        base.submatrix(-n_cut_x+i,n_cut_x+i+1,-n_cut_y+j,n_cut_y+j+1).raw_buffer() ).sum();
    }
  };
